...
<div class="modal fade" id="addModal" data-backdrop="static" data-backdrop="static" tabindex="-1" role="dialog" aria-labelledby="myExtraLargeModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" style="font-weight: 600; color: black; font-size: large;">Create Goal</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <i class="fas fa-times" style="font-size: 25px; "></i>
                </button>
            </div>
            <div class="modal-body">
                <form action="{{url('/goal/addGoal')}}" method="post" enctype="multipart/form-data">
                    @csrf
                    <div class="row">
                        ...
                        <div class="col-sm-6">
                            <div class="form-group">
                                <label style="font-weight: bold;" for="role">Goal Status <span style="color: red;">&#42</span></label>
                        ...
                        <tr>
                            ...
                            <div class="modal fade" id="updateModal{{$data->id}}" data-backdrop="static" data-backdrop="static" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                <div class="modal-dialog modal-lg">
                                    <div class="modal-content">
                                        <div class="modal-header">
                                            <h5 class="modal-title" style="font-weight: 600; color: black; font-size: large;">Update Goal</h5>
                                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                                <i class="fas fa-times" style="font-size: 25px; "></i>
                                            </button>
                                        </div>
                                        <div class="modal-body" style="color: black;">
                                            <form action="{{url('/goal/updateGoal')}}" method="post" enctype="multipart/form-data">
                                                @csrf
                            ...
                            <div class="modal fade" id="deleteModal{{$data->id}}" data-backdrop="static" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                        ...
                                        <form action="{{url('/goal/deleteGoal')}}" method="post">
                                            @csrf
                                            <input type="hidden" name="hiddenId" value="{{$data->id}}">
                                            <div class="modal-body">
                                                <p style="color: black;"> Are you sure you want to delete this Goal? <br> ACTION CAN NOT BE REVERTED </p>
...
<script>
    function checkName() {
        ...
        var errorTitle = document.getElementById("nameError");

        if (namee != "") {
            // ajax
            $.ajax({
                type: "GET",
                url: "{{url('/goal/checkName')}}",
                data: {
                    'name': namee,
                },
                dataType: "json",
                success: function(response) {
                    errorTitle.innerHTML = "Name already exists";
                    document.getElementById("Name").style.borderColor = "red";
                    errorTitle.style.color = "red";
                    document.getElementById("addBtn").disabled = true;
                },
                error: function(response) {
                    errorTitle.innerHTML = "Name is available";
            ...
            document.getElementById("Name").style.borderColor = "red";
            errorTitle.innerHTML = "Enter a name";
            errorTitle.style.color = "red";
            document.getElementById("addBtn").disabled = true;
        }
    }
    ...
    description.addEventListener('invalid', function(event) {
        if (event.target.validity.valueMissing) {
            event.target.setCustomValidity('Goal is required');
        } else if (event.target.validity.tooShort) {
            event.target.setCustomValidity('Goal Too short');
        }
    })
...
<script>
    function getId(id) {
        ...
        enamee.addEventListener('invalid', function(event) {
            if (event.target.validity.valueMissing) {
                event.target.setCustomValidity('Goal Name is required');
            } else if (event.target.validity.tooShort) {
                event.target.setCustomValidity('Goal Name Too short');
